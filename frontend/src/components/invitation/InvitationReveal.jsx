...
// ─────────────────────────────────────────────────────────────────────────────
export default function InvitationReveal({ styleKey, eventType, names = "", initials = "♥", onDone }) {
    ...
    <AnimatePresence>
      {!gone && (
        <motion.div className="fixed inset-0 z-[80] overflow-hidden select-none" data-testid="invitation-reveal"
          ...
          <Scene open={open} names={names} initials={initials} />
...
// ══ 1) MÜHÜRLÜ ZARF ══════════════════════════════════════════════════════════
function EnvelopeScene({ open, names, initials }) {
  const paper = "radial-gradient(120% 120% at 30% 20%, #fbf6ec, #efe4d0 60%, #e4d5ba)";
  return (
    <div className="absolute inset-0 grid place-items-center px-6"
      style={{ background: "radial-gradient(130% 120% at 50% 0%, #7d1f1f, #4a1111 55%, #2c0a0a)" }}>
      ...
      <motion.div className="absolute z-20 rounded-md overflow-hidden"
        ...
        <div className="h-full w-full flex flex-col items-center justify-center text-center px-8">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: open ? 1 : 0 }} transition={{ delay: 1.9, duration: 0.8 }}>
            <div className="text-[11px] tracking-[0.45em] uppercase mb-5" style={{ color: "#9a6a2f" }}>Davetlisiniz</div>
            <div className="mx-auto mb-5 w-14 h-14 rounded-full grid place-items-center" style={{ border: "1.5px solid #c8a24a", color: "#7d1f1f", fontFamily: "'Cardo', serif", fontSize: "1.4rem" }}>{initials}</div>
            <div style={{ fontFamily: "'Cardo', serif", color: "#4a2020", fontSize: "clamp(2rem,7vw,3.2rem)", lineHeight: 1.1 }}>{names}</div>
            <div className="mx-auto my-5 h-px w-24" style={{ background: "#c8a24a" }} />
            <div className="text-xs tracking-[0.3em] uppercase" style={{ color: "#9a6a2f" }}>sizi aramızda görmekten mutluluk duyarız</div>
          </motion.div>
        </div>
      </motion.div>
      ...
      <div className="relative z-30" style={{ width: "min(88vw, 460px)", height: 300, perspective: 1400 }}>
        <motion.div className="absolute inset-x-0 bottom-0"
          style={{ height: 200, background: "linear-gradient(#8f2626,#6d1616)", boxShadow: "inset 0 8px 30px rgba(0,0,0,.35)", borderRadius: "0 0 8px 8px" }}
        ...
        <motion.div className="absolute inset-x-0 top-0 origin-top z-40"
          style={{ height: 150, background: "linear-gradient(#a12e2e,#7d1f1f)", clipPath: "polygon(0 0, 100% 0, 50% 100%)", transformStyle: "preserve-3d" }}
        ...
        <motion.div className="absolute left-1/2 top-[118px] -translate-x-1/2 z-50 rounded-full grid place-items-center"
          style={{ width: 66, height: 66, background: "radial-gradient(circle at 35% 30%, #b83b3b, #7a1414)", boxShadow: "0 6px 14px rgba(0,0,0,.4)", color: "#f2d9a0", fontFamily: "'Cardo', serif", fontSize: "1.3rem", border: "2px solid #5c0f0f" }}
...
// ══ 2) ÇİÇEKLİ BAHÇE ═════════════════════════════════════════════════════════
function GardenScene({ open, names, initials }) {
  const petals = useMemo(() => Array.from({ length: 16 }, () => ({
    left: rnd(0, 100), size: rnd(14, 30), delay: rnd(0, 1.6), dur: rnd(5, 9), rot: rnd(0, 360),
    c: ["#f4c6d5", "#e79bb4", "#f7e2a3", "#ffffff"][Math.floor(rnd(0, 4))],
  })), []);
  const blooms = useMemo(() => Array.from({ length: 10 }, (_, i) => ({
    x: rnd(-46, 46), y: rnd(-40, 40), s: rnd(0.5, 1.1), delay: 1 + i * 0.12, c: ["#e79bb4", "#f4c6d5", "#f7e2a3"][i % 3],
  })), []);
  return (
...
// ══ 3) IŞILTILI SALON ════════════════════════════════════════════════════════
function BallroomScene({ open, names, initials }) {
  ...
  return (
    <div className="absolute inset-0 grid place-items-center px-6 overflow-hidden"
      ...
      <div className="pointer-events-none absolute inset-0">
        {bokeh.map((bk, i) => (
          <motion.div key={i} className="absolute rounded-full" style={{ left: `${bk.left}%`, top: `${bk.top}%`, width: bk.size, height: bk.size, background: "radial-gradient(circle,#fff5cc,#e6c260)", filter: "blur(1px)" }}
      ...
      {open && (
        <div className="pointer-events-none absolute inset-0">
          {sparks.map((s, i) => (
            <motion.div key={i} className="absolute rounded-full" style={{ left: `${s.left}%`, bottom: -10, width: s.size, height: s.size, background: "#f4dd8f", boxShadow: "0 0 8px #e6c260" }}
      ...
      <motion.div className="relative z-10 text-center px-8"
        ...
        <div className="mx-auto mb-4 w-16 h-16 rounded-full grid place-items-center"
          style={{ border: "1.5px solid #e6c260", color: "#f2dd97", fontFamily: "'Great Vibes', cursive", fontSize: "2rem", boxShadow: "0 0 30px #e6c26066" }}>{initials}</div>
        <motion.div style={{ fontFamily: "'Great Vibes', cursive", fontSize: "clamp(2.8rem,11vw,4.6rem)", lineHeight: 1, backgroundImage: "linear-gradient(90deg,#c8a24a,#fff4cf,#e6c260)", backgroundSize: "200% 100%", WebkitBackgroundClip: "text", backgroundClip: "text", color: "transparent", filter: "drop-shadow(0 2px 14px rgba(230,194,96,.4))" }}
          animate={{ backgroundPositionX: ["0%", "100%"] }} transition={{ duration: 4, repeat: Infinity, ease: "linear" }}>
          {names}
        </motion.div>
        <div className="mt-4 text-xs tracking-[0.4em] uppercase" style={{ color: "#d8c07e" }}>zarif bir gecenin başlangıcı</div>
      </motion.div>
    </div>
  );
}
...
// ══ 4) MODERN MİNİMAL ════════════════════════════════════════════════════════
function MinimalScene({ open, names, initials }) {
  return (
    <div className="absolute inset-0 grid place-items-center px-8" style={{ background: "#0e0e0e" }}>
      <motion.div className="absolute left-0 right-0 h-px" style={{ top: "50%", background: "#e5e5e5" }}
      ...
      <div className="relative z-10 text-center overflow-hidden">
        <motion.div className="text-[11px] tracking-[0.6em] uppercase mb-6" style={{ color: "#8a8a8a" }}
          initial={{ opacity: 0, y: 10 }} animate={{ opacity: open ? 1 : 0, y: open ? 0 : 10 }} transition={{ delay: 0.9, duration: 0.7 }}>
          Davetlisiniz
        </motion.div>
        <div className="overflow-hidden">
          <motion.div style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 800, color: "#fff", fontSize: "clamp(2rem,9vw,4rem)", letterSpacing: "-0.02em", lineHeight: 1.05, textTransform: "uppercase" }}
            initial={{ y: "110%" }} animate={{ y: open ? "0%" : "110%" }} transition={{ delay: 1.1, duration: 0.9, ease: EASE }}>
            {names}
          </motion.div>
        </div>
        <motion.div className="mx-auto mt-6 h-8 w-px" style={{ background: "#4a4a4a" }}
          initial={{ scaleY: 0 }} animate={{ scaleY: open ? 1 : 0 }} transition={{ delay: 1.8, duration: 0.6 }} />
        <motion.div className="mt-6 text-xs tracking-[0.45em] uppercase" style={{ color: "#9a9a9a" }}
