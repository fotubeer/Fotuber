...
export default function AdminPhotobooth() {
  ...
  const [tplForm, setTplForm] = useState({ name: "", layout: "strip4", accent: "#111827" });
  ...
  const addTemplate = async () => {
    ...
    try { await api.post("/photobooth/admin/templates", { ...tplForm, category: tplForm.layout, sort: templates.length }); setTplForm({ name: "", layout: "strip4", accent: "#111827" }); loadAll(); }
    catch (e) { toast.error(formatApiError(e)); }
  };
    ...
    <div className="space-y-6" data-testid="admin-photobooth">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Kiosk yazılımı taslağı — çerçeveler, paketler ve marka ayarları. (AI/PayTR/donanım sonraki fazda)</p>
        </div>
      ...
      <div className="flex items-start gap-2 rounded-lg bg-amber-50 border border-amber-200 p-3 text-sm text-amber-800">
        ...
        <span>Bu bir <b>taslak</b>. Akış: Geri sayım → çekim → filtre → çerçeve → paket → QR. Ödeme adımı şimdilik atlandı; PayTR fiziksel POS, AI arka plan silme, video/GIF ve Anı Duvarı sonraki fazlarda eklenecek.</span>
      </div>
