...
export default function AdminAdBanners() {
  ...
  const upload = async () => {
    if (!file) { toast.error("Bir görsel/GIF seçin"); return; }
    setUploading(true);
    try {
      ...
      setForm({ title: "", target_url: "", placement: form.placement, orientation: "horizontal", sort: 0 });
  ...
  const toggle = async (b) => {
    await api.put(`/admin/ad-banners/${b.id}`, { title: b.title, target_url: b.target_url, placement: b.placement, orientation: b.orientation, active: !b.active, sort: b.sort });
    load();
  };
