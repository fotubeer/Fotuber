...
- **Vesikalık — Rötuş Fırçası & Cilt Yumuşatma Fix**: `RetouchBrush.jsx` baştan yazıldı: (1) MAX_EDGE=1600 downscale ile büyük telefon fotoğraflarında bile responsive, (2) `crossOrigin` attribute kaldırıldı (data URL için gereksizdi ve garip renk fallback'lerine yol açıyordu), (3) canvas explicit CSS boyutuyla render ediliyor (max 720×480), (4) `useLayoutEffect` benzeri erken load state için "Fotoğraf yükleniyor…" spinner + `visibility:hidden` canvas — hazır olana kadar hiçbir pointer olayı işlenmiyor, (5) `pushHistory` try/catch ile sarıldı. Test edildi: click sonrası kanvas'a gerçek beyaz daire boyandı. `drawSingle`'daki cilt yumuşatma (retouch) `ctx.filter="blur(2px)"` yerine offscreen canvas + `blur(5px)` + %55 alpha bindirme ile yeniden yazıldı; Safari fallback olarak downscale→upscale trick. Efekt artık görünür.
